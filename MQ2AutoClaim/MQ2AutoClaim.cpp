...
MQ2AutoClaim  - Claim your free station cash
   ...
   Original Author notes: This was originally a macro, it was converted to a plugin so it will automatically run at startup.
	...
	Update 2/25/2021 - Chatwiththisname
		...
		adjusted spacing and put conditions on seperate lines from the actions.
...
void LoadINI();
CSidlScreenWnd* MarketWnd;
CXWnd* Funds;
CStmlWnd* Desc;
CSidlScreenWnd* PopupWnd;
...
PLUGIN_API VOID OnPulse()
{
	...
	if (bdebugging)
		WriteChatf("PluginState: %i", PluginState);
	...
	if (!MarketWnd || !Funds || !Desc || !PopupWnd)
		return;
	...
	switch (PluginState) {
		...
		case 2: // Wait for market place window to open and populate
			...
			// "..." seen on 2/25/2021 when marketplace was not loading current station cash. It's populated, but not valid info.
			if (!szCash[0] || !_stricmp(szCash, "..."))
				return;

			GetCXStr(Desc->STMLText, szDesc, MAX_STRING);
			if (bdebugging)
				WriteChatf("Desc: %s", szDesc);
		...
		case 3:	// Wait for funds to update
		{
			...
			if (bdebugging)
				WriteChatf("Comparing Funds. Current: %s, Previous: %s", sztemp, szCash);
...
void LoadINI() {
	...
	if (temp != -1) {
		...
		if (bdebugging)
			WriteChatf("INI Entry was already present ->temp: %i. Setting: %s",temp, (bDiscardPopup ? "\agOn" : "\arOff"));
	}
	else {
		WritePrivateProfileString("Settings", "AutoClosePopup", "0", INIFileName);
		if (bdebugging)
			WriteChatf("INI entry was \arnot\ax present ->temp: %i. Setting: %s", temp, (bDiscardPopup ? "\agOn" : "\arOff"));
	}
}