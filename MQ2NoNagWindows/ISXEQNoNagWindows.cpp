...
#include "ISXEQNoNagWindows.h"

// The mandatory pre-setup function.  Our name is "ISXEQNoNagWindows", and the class is ISXEQNoNagWindows.
...
// our instanced class.
ISXPreSetup("ISXEQNoNagWindows",ISXEQNoNagWindows);
...
unsigned int ISXEQNoNagWindowsXML=0;
...
bool ISXEQNoNagWindows::Initialize(ISInterface *p_ISInterface)
...
WriteChatf("ISXEQNoNagWindows Loaded");
return true;
}

// shutdown sequence
void ISXEQNoNagWindows::Shutdown()
{
    // save settings, if you changed them and want to save them now.  You should normally save
    // changes immediately.
    //pISInterface->ExportSet(ISXEQNoNagWindowsXML,XMLFileName);

    pISInterface->UnloadSet(ISXEQNoNagWindowsXML);
...
void ISXEQNoNagWindows::ConnectServices()
{
...
void ISXEQNoNagWindows::RegisterCommands()
{
    // add any commands
...
void ISXEQNoNagWindows::RegisterAliases()
{
    // add any aliases
}

void ISXEQNoNagWindows::RegisterDataTypes()
{
    // add any datatypes
    // pMyType = new MyType;
...
void ISXEQNoNagWindows::UnRegisterCommands()
{
    // remove commands
    //    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQNoNagWindows::UnRegisterAliases()
{
    // remove aliases
}
void ISXEQNoNagWindows::UnRegisterDataTypes()
{
    // remove data types
    //if (pMyType)
    //{
...
void ISXEQNoNagWindows::UnRegisterTopLevelObjects()
{
    // remove Top-Level Objects
    //    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQNoNagWindows::UnRegisterServices()
{
    // shutdown our own services
    //    if (hISXEQNoNagWindowsService)
    //        pISInterface->ShutdownService(this,hISXEQNoNagWindowsService);
}

void ISXEQNoNagWindows::LoadSettings()
{
    bool bChanged=false;

    // open the settings file, and store the ID number in ISXEQNoNagWindowsXML
    ISXEQNoNagWindowsXML=pISInterface->OpenSettings(XMLFileName);
    ...
    Any set can be referred to by its ID.  The base set is stored in ISXEQNoNagWindowsXML.  Note that the settings
    ...
    */
    ...
    if (unsigned int GeneralGUID=pISInterface->FindSet(ISXEQNoNagWindowsXML,"General"))
    {
