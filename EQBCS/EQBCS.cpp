...
*
* Version 1.3 - 20141014
* - Added support for /bcg send message to everyone in group.
*
*/ 
...
public: // Vars
  ...
  CClientNode *next;
  time_t lastPingSecs;
  time_t lastPingReponseTimeSecs;
public:
...
void CEqbcs::HandleNewClient(struct sockaddr_in *sockAddress)
{
  // Open the new socket as 'new_socket'
	...
	sprintf_s(buf, "-- Client connection: fd %d\n", iSocketHandle);
	...
	sprintf_s(buf, (char *)"Denied - too many connections");
  ...
  if (cn_to!=NULL) {
    ...
    for (cn_to=clientList; cn_to!=NULL; cn_to=cn_to->next) {
      if((cn->bLocalEcho || cn_to!=cn) && cn_to->chanList!=NULL) {
        strncpy_s(szTemp,cn_to->chanList,2047);
		szTemp[2047] = '\0';
        token=strtok(szTemp," \n");
        while (token!=NULL) {
  ...
  for (CClientNode *cn=clientList; cn != NULL; cn = cn->next)  {
    if (cn != cnCheck && strcmp(cn->szCharName, cnCheck->szCharName) == 0) {
      ...
      if (strlen(cn->szCharName) < CClientNode::MAX_CHARNAMELEN-5) {
        strcat_s(cn->szCharName, cn->MAX_CHARNAMELEN, "-old");
      }
    }
  }
}
