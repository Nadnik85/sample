...
#include "ISXEQThrowIt.h"

// The mandatory pre-setup function.  Our name is "ISXEQThrowIt", and the class is ISXEQThrowIt.
...
// our instanced class.
ISXPreSetup("ISXEQThrowIt",ISXEQThrowIt);
...
unsigned int ISXEQThrowItXML=0;
...
bool ISXEQThrowIt::Initialize(ISInterface *p_ISInterface)
...
WriteChatf("ISXEQThrowIt Loaded");
return true;
}

// shutdown sequence
void ISXEQThrowIt::Shutdown()
{
    // save settings, if you changed them and want to save them now.  You should normally save
    // changes immediately.
    //pISInterface->ExportSet(ISXEQThrowItXML,XMLFileName);

    pISInterface->UnloadSet(ISXEQThrowItXML);
...
void ISXEQThrowIt::ConnectServices()
{
...
void ISXEQThrowIt::RegisterCommands()
{
    // add any commands
...
void ISXEQThrowIt::RegisterAliases()
{
    // add any aliases
}

void ISXEQThrowIt::RegisterDataTypes()
{
    // add any datatypes
    // pMyType = new MyType;
...
void ISXEQThrowIt::UnRegisterCommands()
{
    // remove commands
    //    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQThrowIt::UnRegisterAliases()
{
    // remove aliases
}
void ISXEQThrowIt::UnRegisterDataTypes()
{
    // remove data types
    //if (pMyType)
    //{
...
void ISXEQThrowIt::UnRegisterTopLevelObjects()
{
    // remove Top-Level Objects
    //    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQThrowIt::UnRegisterServices()
{
    // shutdown our own services
    //    if (hISXEQThrowItService)
    //        pISInterface->ShutdownService(this,hISXEQThrowItService);
}

void ISXEQThrowIt::LoadSettings()
{
    bool bChanged=false;

    // open the settings file, and store the ID number in ISXEQThrowItXML
    ISXEQThrowItXML=pISInterface->OpenSettings(XMLFileName);
    ...
    Any set can be referred to by its ID.  The base set is stored in ISXEQThrowItXML.  Note that the settings
    ...
    */
    ...
    if (unsigned int GeneralGUID=pISInterface->FindSet(ISXEQThrowItXML,"General"))
    {
