...
int __declspec(naked) MapViewMap__HandleRButtonDown(CXPoint& point, int unknown)
{
	__asm {
		push ecx;
		push eax;
...
public:
    ...
    int HandleLButtonDown_Detour(class CXPoint& point, unsigned __int32 flags) {
        ...
        if (pZone && pDsp) {
            FLOAT curr_z = pDsp->GetFloorHeight(points[0], points[1], pZone->Ceiling - 2.f, 0.f, CVector3(0, 0, -1), NULL, Undefined, 0.f);
            do {
                z_hits.push_back(curr_z);
                curr_z = pDsp->GetFloorHeight(points[0], points[1], curr_z - 2.f, 0.f, CVector3(0, 0, -1), NULL, Undefined, 0.f);
