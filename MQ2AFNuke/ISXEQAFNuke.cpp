...
#include "ISXEQAFNuke.h"

// The mandatory pre-setup function.  Our name is "ISXEQAFNuke", and the class is ISXEQAFNuke.
...
// our instanced class.
ISXPreSetup("ISXEQAFNuke", ISXEQAFNuke);
...
bool ISXEQAFNuke::Initialize(ISInterface *p_ISInterface)
{
	...
	WriteChatf("ISXEQAFNuke Loaded");
	return true;
}

// shutdown sequence
void ISXEQAFNuke::Shutdown()
{
	// save settings, if you changed them and want to save them now.  You should normally save
	// changes immediately.
	//pISInterface->ExportSet(ISXEQAFNukeXML,XMLFileName);

	pISInterface->UnloadSet(ISXEQAFNukeXML);
...
void ISXEQAFNuke::ConnectServices()
{
...
void ISXEQAFNuke::RegisterCommands()
{
	// add any commands
...
void ISXEQAFNuke::RegisterAliases()
{
	// add any aliases
}

void ISXEQAFNuke::RegisterDataTypes()
{
	// add any datatypes
	// pMyType = new MyType;
...
void ISXEQAFNuke::UnRegisterCommands()
{
	// remove commands
	//    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQAFNuke::UnRegisterAliases()
{
	// remove aliases
}
void ISXEQAFNuke::UnRegisterDataTypes()
{
	// remove data types
	//if (pMyType)
	//{
...
void ISXEQAFNuke::UnRegisterTopLevelObjects()
{
	// remove Top-Level Objects
	//    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQAFNuke::UnRegisterServices()
{
	// shutdown our own services
	//    if (hISXEQAFNukeService)
	//        pISInterface->ShutdownService(this,hISXEQAFNukeService);
}

void ISXEQAFNuke::LoadSettings()
{
	bool bChanged = false;

	// open the settings file, and store the ID number in ISXEQAFNukeXML
	ISXEQAFNukeXML = pISInterface->OpenSettings(XMLFileName);
	...
	Any set can be referred to by its ID.  The base set is stored in ISXEQAFNukeXML.  Note that the settings
	...
	*/
	...
	if (unsigned int GeneralGUID = pISInterface->FindSet(ISXEQAFNukeXML, "General"))
	{
