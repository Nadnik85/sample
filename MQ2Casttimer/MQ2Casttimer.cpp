			...
			switch(pSpell->SpellType) {
				...
				case 2:  //Beneficial
					if(PlayerHasAAAbility(GetAAIndexByName("Spell Casting Reinforcement Mastery"))) dMod = dMod+50;
						...
						for (unsigned int j=0; j < AA_CHAR_MAX_REAL; j++) {
							if (pPCData->GetAlternateAbilityId(j)) {
								if ( pPCData->GetAlternateAbilityId(j) == GetAAIndexByName("Spell Casting Reinforcement")) {
									switch(GetCharInfo2()->AAList[j].PointsSpent) {
										case 2: // level 1
											dMod = dMod+5;
											break;
		...
		if(PCONTENTS pItem=GetCharInfo2()->pInventoryArray->InventoryArray[Scan]) {
			if(PSPELL fSpell=GetSpellByID(GetItemFromContents(pItem)->Focus.SpellID)) {
				...
				if ( FocusMod && correctType && correctFocus ) {
					if (pSpell->ClassLevel[GetCharInfo()->pSpawn->Class-1] > FocusLevel) FocusMod = FocusMod-((((pSpell->ClassLevel[((PSPAWNINFO)pCharSpawn)->Class-1]-FocusLevel)*FocusLevelDecay)*FocusMod)/100); 
...
VOID ParseSpellsOnce() {
    ...
    for (DWORD dwSpellID = 0; dwSpellID < TOTAL_SPELL_COUNT; dwSpellID++) {
		...
		if(pSpell->ID && AcceptedSpells(pSpell)) {
            Level=0;
            for (int c=1; c<17; c++) {
                if(pSpell->ClassLevel[c-1] <= MAXSPELLLEVEL && pSpell->ClassLevel[c-1] > Level) Level = pSpell->ClassLevel[c-1];
            }
            if(Level) {
