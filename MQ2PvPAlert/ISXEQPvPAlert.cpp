...
#include "ISXEQPvPAlert.h"

// The mandatory pre-setup function.  Our name is "ISXEQPvPAlert", and the class is ISXEQPvPAlert.
...
// our instanced class.
ISXPreSetup("ISXEQPvPAlert",ISXEQPvPAlert);
...
unsigned int ISXEQPvPAlertXML=0;
...
bool ISXEQPvPAlert::Initialize(ISInterface *p_ISInterface)
...
WriteChatf("ISXEQPvPAlert Loaded");
return true;
}

// shutdown sequence
void ISXEQPvPAlert::Shutdown()
{
    // save settings, if you changed them and want to save them now.  You should normally save
    // changes immediately.
    //pISInterface->ExportSet(ISXEQPvPAlertXML,XMLFileName);

    pISInterface->UnloadSet(ISXEQPvPAlertXML);
...
void ISXEQPvPAlert::ConnectServices()
{
...
void ISXEQPvPAlert::RegisterCommands()
{
    // add any commands
...
void ISXEQPvPAlert::RegisterAliases()
{
    // add any aliases
}

void ISXEQPvPAlert::RegisterDataTypes()
{
    // add any datatypes
    // pMyType = new MyType;
...
void ISXEQPvPAlert::UnRegisterCommands()
{
    // remove commands
    //    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQPvPAlert::UnRegisterAliases()
{
    // remove aliases
}
void ISXEQPvPAlert::UnRegisterDataTypes()
{
    // remove data types
    //if (pMyType)
    //{
...
void ISXEQPvPAlert::UnRegisterTopLevelObjects()
{
    // remove Top-Level Objects
    //    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQPvPAlert::UnRegisterServices()
{
    // shutdown our own services
    //    if (hISXEQPvPAlertService)
    //        pISInterface->ShutdownService(this,hISXEQPvPAlertService);
}

void ISXEQPvPAlert::LoadSettings()
{
    bool bChanged=false;

    // open the settings file, and store the ID number in ISXEQPvPAlertXML
    ISXEQPvPAlertXML=pISInterface->OpenSettings(XMLFileName);
    ...
    Any set can be referred to by its ID.  The base set is stored in ISXEQPvPAlertXML.  Note that the settings
    ...
    */
    ...
    if (unsigned int GeneralGUID=pISInterface->FindSet(ISXEQPvPAlertXML,"General"))
    {
