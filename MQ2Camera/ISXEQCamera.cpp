...
#include "ISXEQCamera.h"

// The mandatory pre-setup function.  Our name is "ISXEQCamera", and the class is ISXEQCamera.
...
// our instanced class.
ISXPreSetup("ISXEQCamera", ISXEQCamera);
...
bool ISXEQCamera::Initialize(ISInterface *p_ISInterface)
{
	...
	WriteChatf("ISXEQCamera Loaded");
	return true;
}

// shutdown sequence
void ISXEQCamera::Shutdown()
{
	// save settings, if you changed them and want to save them now.  You should normally save
	// changes immediately.
	//pISInterface->ExportSet(ISXEQCameraXML,XMLFileName);

	pISInterface->UnloadSet(ISXEQCameraXML);
...
void ISXEQCamera::ConnectServices()
{
...
void ISXEQCamera::RegisterCommands()
{
	// add any commands
...
void ISXEQCamera::RegisterAliases()
{
	// add any aliases
}

void ISXEQCamera::RegisterDataTypes()
{
	// add any datatypes
	// pMyType = new MyType;
...
void ISXEQCamera::UnRegisterCommands()
{
	// remove commands
	//    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQCamera::UnRegisterAliases()
{
	// remove aliases
}
void ISXEQCamera::UnRegisterDataTypes()
{
	// remove data types
	//if (pMyType)
	//{
...
void ISXEQCamera::UnRegisterTopLevelObjects()
{
	// remove Top-Level Objects
	//    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQCamera::UnRegisterServices()
{
	// shutdown our own services
	//    if (hISXEQCameraService)
	//        pISInterface->ShutdownService(this,hISXEQCameraService);
}

void ISXEQCamera::LoadSettings()
{
	bool bChanged = false;

	// open the settings file, and store the ID number in ISXEQCameraXML
	ISXEQCameraXML = pISInterface->OpenSettings(XMLFileName);
	...
	Any set can be referred to by its ID.  The base set is stored in ISXEQCameraXML.  Note that the settings
	...
	*/
	...
	if (unsigned int GeneralGUID = pISInterface->FindSet(ISXEQCameraXML, "General"))
	{
