...
// The mandatory pre-setup function.  Our name is "ISXEQKiss", and the class is ISXEQKiss.
...
// our instanced class.
ISXPreSetup("ISXEQKiss",ISXEQKiss);
...
bool ISXEQKiss::Initialize(ISInterface *p_ISInterface)
...
WriteChatf("ISXEQKiss Loaded");
return true;
}

// shutdown sequence
void ISXEQKiss::Shutdown()
{
    // save settings, if you changed them and want to save them now.  You should normally save
    // changes immediately.
    //pISInterface->ExportSet(ISXEQKissXML,XMLFileName);

    pISInterface->UnloadSet(ISXEQKissXML);
...
void ISXEQKiss::ConnectServices()
{
...
void ISXEQKiss::RegisterCommands()
{
    // add any commands
...
void ISXEQKiss::RegisterAliases()
{
    // add any aliases
}

void ISXEQKiss::RegisterDataTypes()
{
    // add any datatypes
    // pMyType = new MyType;
...
void ISXEQKiss::UnRegisterCommands()
{
    // remove commands
    //    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQKiss::UnRegisterAliases()
{
    // remove aliases
}
void ISXEQKiss::UnRegisterDataTypes()
{
    // remove data types
    //if (pMyType)
    //{
...
void ISXEQKiss::UnRegisterTopLevelObjects()
{
    // remove Top-Level Objects
    //    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQKiss::UnRegisterServices()
{
    // shutdown our own services
    //    if (hISXEQKissService)
    //        pISInterface->ShutdownService(this,hISXEQKissService);
}

void ISXEQKiss::LoadSettings()
{
    bool bChanged=false;

    // open the settings file, and store the ID number in ISXEQKissXML
    ISXEQKissXML=pISInterface->OpenSettings(XMLFileName);
    ...
    Any set can be referred to by its ID.  The base set is stored in ISXEQKissXML.  Note that the settings
    ...
    */
    ...
    if (unsigned int GeneralGUID=pISInterface->FindSet(ISXEQKissXML,"General"))
    {
