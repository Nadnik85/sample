...
#include "ISXEQDiscord.h"

// The mandatory pre-setup function.  Our name is "ISXEQDiscord", and the class is ISXEQDiscord.
...
// our instanced class.
ISXPreSetup("ISXEQDiscord", ISXEQDiscord);
...
unsigned int ISXEQDiscordXML = 0;
...
bool ISXEQDiscord::Initialize(ISInterface *p_ISInterface)
{
	...
	WriteChatf("ISXEQDiscord Loaded");
	return true;
}

// shutdown sequence
void ISXEQDiscord::Shutdown()
{
	// save settings, if you changed them and want to save them now.  You should normally save
	// changes immediately.
	//pISInterface->ExportSet(ISXEQDiscordXML,XMLFileName);

	pISInterface->UnloadSet(ISXEQDiscordXML);
...
void ISXEQDiscord::ConnectServices()
{
...
void ISXEQDiscord::RegisterCommands()
{
	// add any commands
...
void ISXEQDiscord::RegisterAliases()
{
	// add any aliases
}

void ISXEQDiscord::RegisterDataTypes()
{
	// add any datatypes
	// pMyType = new MyType;
...
void ISXEQDiscord::UnRegisterCommands()
{
	// remove commands
	//    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQDiscord::UnRegisterAliases()
{
	// remove aliases
}
void ISXEQDiscord::UnRegisterDataTypes()
{
	// remove data types
	//if (pMyType)
	//{
...
void ISXEQDiscord::UnRegisterTopLevelObjects()
{
	// remove Top-Level Objects
	//    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQDiscord::UnRegisterServices()
{
	// shutdown our own services
	//    if (hISXEQDiscordService)
	//        pISInterface->ShutdownService(this,hISXEQDiscordService);
}

void ISXEQDiscord::LoadSettings()
{
	bool bChanged = false;

	// open the settings file, and store the ID number in ISXEQDiscordXML
	ISXEQDiscordXML = pISInterface->OpenSettings(XMLFileName);
	...
	Any set can be referred to by its ID.  The base set is stored in ISXEQDiscordXML.  Note that the settings
	...
	*/
	...
	if (unsigned int GeneralGUID = pISInterface->FindSet(ISXEQDiscordXML, "General"))
	{
